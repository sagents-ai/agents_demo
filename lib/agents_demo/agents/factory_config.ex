defmodule AgentsDemo.Agents.FactoryConfig do
  ...
  From a Phoenix LiveView socket's assigns:
      ...
      AgentsDemo.Agents.FactoryConfig.from_socket_assigns(socket.assigns)
      |> AgentsDemo.Agents.FactoryConfig.build()

  From a neutral map (GenServer, GraphQL resolver, test):

      AgentsDemo.Agents.FactoryConfig.from_inputs(%{
        scope: scope,
  ...
  call site can compose them:
      ...
      AgentsDemo.Agents.FactoryConfig.from_inputs(inputs)
      |> AgentsDemo.Agents.FactoryConfig.with_project(project)
      |> AgentsDemo.Agents.FactoryConfig.build()
  """
  ...
  embedded_schema do
    ...
    # IANA timezone string for the InjectCurrentTime middleware. Defaults
    # to "UTC" so behavior is deterministic when callers don't surface the
    # browser's timezone (test, admin scripts, GenServer-driven flows).
    field :timezone, :string, default: "UTC"

    # Caller-supplied data exposed to tools via LLMChain.custom_context.
