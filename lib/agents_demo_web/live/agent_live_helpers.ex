defmodule AgentsDemoWeb.AgentLiveHelpers do
  ...
  defp auto_wake_for_pending_interrupt(socket) do
    case Coordinator.ensure_agent_session_running(socket.assigns) do
      {:ok, changes} ->
  ...
  `AgentSubscriberSession.handle_hitl_decision/3`.
  """
        ...
        {:resume, accumulated, changes} ->
          case AgentServer.resume(agent_id, accumulated) do
            :ok ->
              socket
              |> assign(changes)
              |> assign(AgentSubscriberSession.hitl_resume_running_changes())
        ...
        {:resume, resume_data, changes} ->
          case AgentServer.resume(agent_id, resume_data) do
            :ok ->
              socket
              |> assign(changes)
              |> assign(AgentSubscriberSession.question_resume_running_changes())
