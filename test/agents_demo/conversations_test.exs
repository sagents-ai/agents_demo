defmodule AgentsDemo.ConversationsTest do
  ...
  describe "append_thinking_message/2" do
    test "creates a thinking block message" do
      conversation = conversation_fixture()

      assert {:ok, message} =
               Conversations.append_thinking_message(conversation.id, "Let me analyze this...")

      assert message.message_type == "assistant"
      assert message.content_type == "thinking"
      assert message.content == %{"text" => "Let me analyze this..."}
      assert message.sequence == 0
    end
  end

  describe "append_image_message/3" do
    test "creates an image message with URL" do
      conversation = conversation_fixture()

      assert {:ok, message} =
               Conversations.append_image_message(conversation.id, "/uploads/image.png")

      assert message.message_type == "assistant"
      assert message.content_type == "image"
      assert message.content == %{"url" => "/uploads/image.png"}
    end

    test "includes alt_text and caption when provided" do
      conversation = conversation_fixture()

      assert {:ok, message} =
               Conversations.append_image_message(conversation.id, "/img.png",
                 alt_text: "Chart",
                 caption: "Sales data"
               )
    ...
    test "accepts custom message_type" do
      conversation = conversation_fixture()
  ...
  describe "append_file_message/4" do
    test "creates a file reference message" do
      conversation = conversation_fixture()

      assert {:ok, message} =
               Conversations.append_file_message(
                 conversation.id,
                 "/tmp/report.pdf",
                 "Q4 Report.pdf"
               )
      ...
      assert message.message_type == "tool"
      assert message.content_type == "file_reference"
      assert message.content == %{"path" => "/tmp/report.pdf", "name" => "Q4 Report.pdf"}
    end

    test "includes size and mime_type when provided" do
      conversation = conversation_fixture()
  ...
  describe "append_structured_data_message/4" do
    test "creates a structured data message" do
      conversation = conversation_fixture()

      data = [["North", 100], ["South", 200]]

      assert {:ok, message} =
               Conversations.append_structured_data_message(
                 conversation.id,
                 "table",
                 data,
      ...
      assert message.message_type == "tool"
      assert message.content_type == "structured_data"
      assert message.content["format"] == "table"
      assert message.content["data"] == data
      assert message.content["columns"] == ["Region", "Sales"]
    end
  end

  describe "append_notification_message/3" do
    test "creates a notification message" do
      conversation = conversation_fixture()

      assert {:ok, message} =
               Conversations.append_notification_message(
                 conversation.id,
                 "Conversation compacted"
               )

      assert message.message_type == "system"
      assert message.content_type == "notification"
      assert message.content == %{"text" => "Conversation compacted", "level" => "info"}
    end
    ...
    test "accepts custom level" do
      ...
      assert message.content["level"] == "warning"
    end
  end

  describe "append_error_message/3" do
    test "creates an error message" do
      conversation = conversation_fixture()

      assert {:ok, message} =
               Conversations.append_error_message(
                 conversation.id,
                 "Connection failed"
               )

      assert message.message_type == "tool"
      assert message.content_type == "error"
      assert message.content == %{"text" => "Connection failed"}
    end

    test "includes code and details when provided" do
      conversation = conversation_fixture()
  ...
  describe "load_display_messages_by_type/3" do
    test "filters messages by content_type" do
      conversation = conversation_fixture()

      text_message_fixture(conversation.id, %{text: "User message"})
      thinking_message_fixture(conversation.id, %{text: "Thinking 1"})
      text_message_fixture(conversation.id, %{text: "Response", message_type: "assistant"})
      thinking_message_fixture(conversation.id, %{text: "Thinking 2"})

      thinking_messages = Conversations.load_display_messages_by_type(conversation.id, "thinking")

      assert length(thinking_messages) == 2
      assert Enum.all?(thinking_messages, &(&1.content_type == "thinking"))
    end

    test "returns empty list when no messages of type exist" do
      conversation = conversation_fixture()
      text_message_fixture(conversation.id, %{text: "Text only"})

      assert [] = Conversations.load_display_messages_by_type(conversation.id, "image")
    end

    test "orders by inserted_at and sequence" do
      conversation = conversation_fixture()

      # Create thinking messages with different sequences
      msg1 = thinking_message_fixture(conversation.id, %{text: "First", sequence: 0})
      msg2 = thinking_message_fixture(conversation.id, %{text: "Second", sequence: 1})

      messages = Conversations.load_display_messages_by_type(conversation.id, "thinking")
